...
class SpriteStrategy:
    ...
    # 的决策（候选 = 可用攻击/最佳状态技/换人/gather，对手响应 = 攻击/防御/状态/换人/聚能
    # 里的前 k 个）。手写的特例（斩杀/防御/撤人/状态反制/道具）在两条路上都保留，
    # 所以同局配对 A/B 量的就是"贪心 vs 规划"本身。
    # **默认开启**（2026-09-21 定）：同局配对 A/B 两组独立种子 0.671 [0.623,0.719] /
    # 0.686 [0.638,0.733]（各 400 局），已知中性对照 0.508 —— 即规划胜过贪心约 17 个点。
    # 代价约 9 倍墙钟（400 局 124s vs 16s），所以批量评估/自博弈任务要留意；
    # 需要旧口径（对照实验、快速冒烟）时显式传 `plan_depth=0`（`eval_expert_change --ab plan`
    # 的旧臂就是这么做的）。BC 数据现在由**规划层专家**生成，与旧数据分布不同。
    plan_depth: int = 1            # 视野回合数（1 = 只看本回合）
