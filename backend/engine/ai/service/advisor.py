"""backend/engine/ai/service/advisor.py — 实战 PVP 单回合建议（部署侧）
...
def _estimate_win_prob(battle: "Battle", model: "ModularBattleNet", device: str) -> float:
    ...
    """
    from backend.engine.ai.core.evaluator import _state_dict_to_tensors
    state_dict = encode_battle_state(battle)
    tensors = _state_dict_to_tensors(state_dict, device)
    with torch.no_grad():
        value, _ = model(tensors)
