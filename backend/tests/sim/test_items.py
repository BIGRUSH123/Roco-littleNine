...
def test_evolution_power_evolves_to_leader_form():
    ...
    # 断言物种身份（不耦合展示名规则：首领形态的 display_name 现在带（首领形态）后缀）
    assert sprite.name == boss.name
    assert sprite.species.number == boss.number
...
def test_mutable_state_restore_reverts_item_use_and_evolution():
    """MCTS 回滚应恢复道具次数与进化造成的形态/属性变化。"""
    from backend.sim.battle import Battle
    ...
    sprite.bloodline = "首领"
    original_species = sprite.species
    original_stats = dict(sprite.initial_stats)
    original_max_hp = sprite.max_hp
    saved = battle.save_mutable_state()

    result = battle._resolve_item("A")
    assert result == "进化之力"
    assert sprite.species is not original_species
    assert p1.item.uses == 1

    battle.restore_mutable_state(saved)

    assert sprite.species is original_species
    assert sprite.initial_stats == original_stats
    assert sprite.max_hp == original_max_hp
    assert p1.item is not None
    assert p1.item.uses == 0
    assert p1.item.last_use_turn == 0


def test_mutable_state_restore_reverts_wish_skill_replacement():
    """MCTS 回滚应恢复愿力替换的一技能和道具冷却状态。"""
    from backend.sim.battle import Battle
    ...
    sprite = battle.player_a.active
    original_skill = sprite.skills[0]
    saved = battle.save_mutable_state()

    result = battle._resolve_item("A")
    assert result.startswith("愿力(")
    assert sprite.skills[0] is not original_skill
    assert p1.item.uses == 1

    battle.restore_mutable_state(saved)

    assert sprite.skills[0] is original_skill
    assert [bs.name for bs in sprite.skills] == ["猛烈撞击", "甩水"]
    assert p1.item is not None
    assert p1.item.uses == 0
    assert p1.item.last_use_turn == 0
...
if __name__ == "__main__":
    ...
    test_mutable_state_restore_reverts_item_use_and_evolution()
    test_mutable_state_restore_reverts_wish_skill_replacement()
    print("\n  [ALL TESTS PASSED]")
