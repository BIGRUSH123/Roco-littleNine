"""dbg_py_dmg — 打印指定回合所有 take_damage 调用。

用法：env\\python.exe native/tools/dbg_py_dmg.py <spec_id> <turn>
"""
...
orig = Sprite.take_damage


def patched(self, dmg):
    r = orig(self, dmg)
    if battle.turn == turn:
        print(f"t{battle.turn} {self.name} take {dmg} (actual={r}) hp={r and self.current_hp}")
        st = "".join(traceback.format_stack(limit=8)[:-1])
        for line in st.splitlines():
            if "backend" in line and "test_rust_gate" not in line:
                print("    ", line.strip())
    return r


Sprite.take_damage = patched
