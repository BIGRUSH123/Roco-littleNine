...
- **愿力的使用时机（RuleAgent 待改）**：wiki 语义是"血脉属性克制对面时使用"（愿力冲击的属性由血脉决定），
  而现有专家规则是"`hp < 50%` 就用"（`agent_v2.py:174`；`agent.py:96` 还多一个 `aggression > 0.4`），
  既不看换出的技能质量也不看对位克制关系。

### 1.5 引擎与 wiki 的语义偏差（2026-09-20 核对，按优先级）

| 机制 | wiki 语义（权威） | 引擎现状 | 处理 |
|---|---|---|---|
| 道具消耗回合 | 不消耗回合 | `_select_action` 道具循环（结算后重选行动） | ✅ 已一致 |
| 愿力效果 | 本回合**第一个技能替换为「愿力冲击」**：2 费 / 80 威力 / **属性=血脉** / 类型=物攻魔攻取高 / **应对状态时威力 +150%（=200）**（wiki 05 与 `技能图鉴/普通/愿力冲击`） | `_resolve_item` 把槽 0 换成**该血脉的血液技能**（威力随技能，如 升龙咆哮 魔攻 200） | ❌ 需对齐：新建 `data/skills/愿力冲击.json` + 动态属性（=血脉）支持 |
| 愿力数据 | 有 wiki 页；线上 `模块:Pets/data/Skills` **没有**该技能条目（只在裁决/滋养/点燃等特性描述里出现"首个技能替换为××系愿力冲击"） | `data/skills/` 无此技能 | 按 wiki 页手工建数据（属性动态） |
| 首领化收益 | 六维提升 + 特性增强 + **部分精灵技能槽增加**（如独角兽 4→7 技能） | 只提升六维/特性，技能槽固定 4 | ❌ 偏差（影响首领化收益建模），待定 |
| 可首领化精灵 | 本地 wiki 页写"仅 24 只" | 数据里 105 个编号有首领形态、池内 37 只可用 | 本地页过时，以线上模块为准（待抽样核对） |
