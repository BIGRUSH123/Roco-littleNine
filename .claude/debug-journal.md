...
## 2026-05-29 - 被应对技能穿透全流程：伤害/trait/效果照常触发

- **现象**: 攻击技能被对方防御技能应对后，攻击方仍然造成伤害、施加 debuff、获得自身 buff 和 trait 效果（如全属性+20%、回能），打断只在应对方行动后才生效
- **根因**: `battle.py:534` 的 `is_countered` 分支仅追加应对日志，无 `return`——被应对技能穿透能耗支付、奉献消耗、伤害计算、trait observer、技能 VM 全量执行。应对方注射效果（原 670-687）在打断门之后，时序错位
- **修复**: 应对日志后立即注射应对方直接效果 → 扣除基础能耗 → `return events`。阻断被应对技能的后续全部效果执行，但仍消耗能量
- **涉及文件**: backend/sim/battle.py:533-575
- **教训**: 流程中看到"应对成功"日志但技能效果照常执行时，直接查 `_execute_skill_vm` 中 `is_countered` 分支是否有 `return`——日志在前、无阻断，后续代码全部放行
